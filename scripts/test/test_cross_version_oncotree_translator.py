...
class TestCrossVersionOncotreeTranslator(unittest.TestCase):
    ...
    def test_convert_to_target_oncotree_code_forwards_history(self):
        actual_oncotree_codes = convert_to_target_oncotree_code("SEZS", self.original_version, self.latest_version)
    ...
    def test_convert_to_target_oncotree_code_forwards_revocations(self):
        actual_oncotree_codes = convert_to_target_oncotree_code("PTCL", self.original_version, self.latest_version)
    ...
    def test_convert_to_target_oncotree_code_forwards_precursors(self):
        actual_oncotree_codes = convert_to_target_oncotree_code("CTCL", self.original_version, self.latest_version)
        expected_oncotree_codes = ["MYCF"]
        self.assertEqual(expected_oncotree_codes, actual_oncotree_codes)
       
    def test_convert_to_target_oncotree_code_forwards_precursors_and_revocations(self):
        actual_oncotree_codes = convert_to_target_oncotree_code("BALL", self.original_version, self.latest_version)
        expected_oncotree_codes = ["BLL"]
        self.assertEqual(expected_oncotree_codes, actual_oncotree_codes)

    def test_convert_to_target_oncotree_code_backwards_history(self):
        actual_oncotree_codes = convert_to_target_oncotree_code("SS", self.latest_version, self.original_version)
        expected_oncotree_codes = ["SEZS"]
        self.assertEqual(expected_oncotree_codes, actual_oncotree_codes)

    def test_convert_to_target_oncotree_code_backwards_revocations(self):
        actual_oncotree_codes = convert_to_target_oncotree_code("PTCL", self.latest_version, self.original_version)
    ...
    def test_convert_to_target_oncotree_code_backwards_precursors(self):
        actual_oncotree_codes = convert_to_target_oncotree_code("MYCF", self.latest_version, self.original_version)
        expected_oncotree_codes = ["MYCF", "CTCL"] # TODO convert to sets, order doesn't matter?, otherwise make sure order is right
        self.assertEqual(expected_oncotree_codes, actual_oncotree_codes)
       
    def test_convert_to_target_oncotree_code_backwards_precursors_and_revocations(self):
        actual_oncotree_codes = convert_to_target_oncotree_code("BLL", self.latest_version, self.original_version)
        expected_oncotree_codes = ["ALL", "BALL"] # TODO order?
        self.assertEqual(expected_oncotree_codes, actual_oncotree_codes)
